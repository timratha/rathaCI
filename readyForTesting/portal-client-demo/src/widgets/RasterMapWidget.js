define([
    ...
    "dojo/dom-class",
    "dojo/date"
], function (WidgetsInTemplateMixin, TemplatedMixin, WidgetBase,on,declare,lang,template, nlsRasterMapWidget,Deferred,DeferredList,request,has,domGeometry,domStyle,domClass,dojoDate) {
    return declare([WidgetBase, TemplatedMixin, WidgetsInTemplateMixin], {
		    ...
		    postCreate: function () {
				...
				this.ts_id = 2825010;
				this.set("timerange", {from:"2015-07-06T10:00:00",to:"2015-07-06T12:10:00"});
			    
                this.rasterProvider = "http://radar.nimes.fr/RasterProvider/RasterProvider";
                this.mapserver =  "http://radar.nimes.fr/mapserver/mapserv.exe?map=C:\\kisters\\map\\raster.map";
            ...
            resize: function (args) {
				...
				if(args)
				{
					domStyle.set(this.mapNode, "height" , args.h+ "px");
					domStyle.set(this.mapNode, "width" ,args.w+ "px");
				}
				else
				{
					this.inherited(arguments);
				...
				if(this._oMap)
				{
					this._oMap.setSize([size.w,size.h]);
					console.debug("_oMap.setSize", size.w,size.h);
				}
				...
				return;
				   ...
				   console.debug(args);
					...
					console.debug(this.mapNode);
					
				var mapSize = domGeometry.getContentBox(this.containerNode);
				console.debug("mapNode",mapSize);
					
				var mapSize = domGeometry.getContentBox(this.containerNode);
				console.debug("containerNode",mapSize);
					
						var mapSize = domGeometry.getContentBox(this.domNode);
				console.debug("domNode",mapSize);
			...
			//	this.inherited(arguments);
				...
				var size = domGeometry.getContentBox(this.domNode),
					...
					console.debug(size);
					console.debug(this.containerNode);
					console.debug(cont_cb);
					alert(size.h );
					alert(size.w );
					
                size.h = Math.min(size.h, cont_cb.h);
                size.w = Math.min(size.w, cont_cb.w);

				
				if(size.w <=0)
					 size.w = (size.h >0 ) ? parseInt(size.h* 3/2): 600; 
				if(size.h <=0)
					size.h = (size.w >0 ) ? parseInt(size.w*2/3): 400; 
					
					M=this;
				//domGeometry.setMarginBox(this.mapNode, size);
				domStyle.set(this.mapNode, "height" , size.h+ "px");
				domStyle.set(this.mapNode, "width" ,size.w+ "px");
